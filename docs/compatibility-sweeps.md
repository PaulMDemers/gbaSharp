...
- Tactics Ogre Gaiden (`TOGRE GAIDEN`, `ATOJ`, index 219) is now classified as a focused no-BIOS startup divergence rather than simple route noise. The no-BIOS gameplay probe (`artifacts/tactics-ogre-gaiden-nobios-gameplay-20260624.csv`) reaches all four phase frame targets but stays on two black/blank frame hashes, and generic captures under `artifacts/tactics-ogre-gaiden-route-probe-20260624` confirm the final frame is black. The real-BIOS aligned repro (`artifacts/tactics-ogre-gaiden-realbios-gameplay-20260624.csv`) is clean, 4/4 boot rows with animated input phases. Focused traces show no locked-BIOS reads and matching early `CpuSet` SWI parameters; no-BIOS and real-BIOS match through about frame 40, then no-BIOS diverges by frame 60 into the `0800A788`/`r12=03000030` intro-object loop while real-BIOS remains in the normal `080007xx` dispatcher with `r12=030025A0` and later enables `DISPCNT=0x1360`. Treat this as a no-BIOS IRQ/dispatcher/state-lifetime target, not a missing input script.
