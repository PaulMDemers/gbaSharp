...
The current 46-route deep-gameplay rollup is `artifacts/deep-gameplay-current-46-rollup-20260628/summary.md`. It covers 46/46 manifest rows with 0 missing routes, 0 failing rows, and 0 low-diversity warnings after aligning route-specific thresholds to the current repeated evidence. The rollup reports 46 strict `pass, match` rows. The expansion rows add Doom II at frame 6,000 as another first-person raycast/3D engine route, verified by `artifacts/deep-gameplay-current-doomii-gameplay-strict-20260628b/deep-gameplay.csv` with 8 distinct PCs, and Chessmaster at frame 4,800 as an EEPROM board-game UI/rendering route, verified by `artifacts/deep-gameplay-current-chessmaster-gameplay-strict-20260628/deep-gameplay.csv` with 4 distinct PCs.
