...
The real-BIOS gameplay slice for curated indexes 61-80 (`compat-curated-bios-gameplay-061-080-after-alignfix-20260523`) reports 77/80 boot rows, 0 static rows, and 0 timeouts. The only crash is `Banjo-Pilot` (`BANJO PILOT`, `BAJE`) in all input-bearing phases at frame 202. The focused trace `banjopilot-realbios-frame202-tail-20260523.txt` shows generated ARM code branching from `03005A78` to `030052F4`; the helper ends with `ldmia sp!, {lr}; bx lr` and pops `lr=00000001` from `03007E74`, falling through low-memory Thumb. A zeroed EEPROM repro (`banjopilot-zero-eeprom-repro-20260523.txt`) has the same signature, so this is not simply blank save data.
