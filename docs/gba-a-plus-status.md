...
The current post-Tactics compatibility refresh reran the first deep-gameplay chunk under real BIOS. `artifacts/deep-gameplay-suite-current-20260625/deep-gameplay.csv` confirms Doom, GTA, Sonic Advance, and Mario Kart reach live gameplay scenes; Metroid Fusion reached frame 21,802 under a deliberately bounded 600-second foreground cap and should be rerun as its own long route when wall time is available. After visual review, GTA, Sonic Advance, and Mario Kart baselines were refreshed to the current active-gameplay frames, and `artifacts/deep-gameplay-current-01-04-verify-20260625/deep-gameplay.csv` verifies the four completed routes with 4/4 `pass, match` rows.
