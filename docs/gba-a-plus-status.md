...
Save-assisted deep gameplay coverage now has its own strict manifest, `docs/gba-save-assisted-deep-routes.csv`, using read-only approved save fixtures. The stable rows cover EEPROM (`zelda-minish-save-bedroom`, 16,000 frames, 26 snapshots, 10 distinct PCs), Flash64K (`advance-wars-save-map`, 15,000 frames, 25 snapshots, 11 distinct PCs), SRAM (`metroid-fusion-save-gameplay`, 30,000 frames, 25 snapshots, 17 distinct PCs), and a save-loaded Sonic timing/rendering anchor (`sonic-advance-save-demo`, 3,600 frames, 12 snapshots, 8 distinct PCs). The suite wrapper now accepts alternate ROM/baseline roots and emits a contact sheet automatically. The strict run `deep-gameplay-save-assisted-suite-sonic-20260531/deep-gameplay.csv` verifies 4/4 exact baseline matches, and `deep-gameplay-save-assisted-suite-sonic-20260531/contact-sheet.png` visually confirms the Zelda bedroom, Advance Wars map, Metroid boss-room, and Sonic title-demo gameplay checkpoints. `scripts/new-deep-gameplay-frame-candidates.py` now captures candidate stop frames for a manifest row and generates a contact sheet before promotion, which caught title/publisher screens during Sonic route selection. Ruby's longer Flash128K route remains covered by the visual-save tooling but is not part of this strict deep suite yet because its current route budget is too slow for routine regression runs.
