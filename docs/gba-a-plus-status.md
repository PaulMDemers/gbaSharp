...
Longer non-baseline gameplay soak coverage now starts in `docs/gba-longplay-routes.csv`, with status tracked in `docs/gba-longplay-status.md`. The first post-audio longplay smoke reaches extended save-assisted gameplay windows for Sonic Advance, Mario Kart, Metroid Fusion, and Tony Hawk Pro Skater 2 with 4/4 pass rows, 9-17 distinct PCs, and contact-sheet-confirmed gameplay scenes.
