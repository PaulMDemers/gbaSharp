...
Save-assisted deep gameplay coverage now has its own strict manifest, `docs/gba-save-assisted-deep-routes.csv`, using read-only approved save fixtures. The first stable rows cover EEPROM (`zelda-minish-save-bedroom`, 18,000 frames, 30 snapshots, 10 distinct PCs), Flash64K (`advance-wars-save-map`, 15,000 frames, 25 snapshots, 11 distinct PCs), and SRAM (`metroid-fusion-save-gameplay`, 42,000 frames, 35 snapshots, 21 distinct PCs). Zelda and Advance Wars baselines were promoted in `deep-gameplay-save-assisted-zelda-20260530/deep-gameplay.csv` and `deep-gameplay-save-assisted-advance-wars-20260530/deep-gameplay.csv`; Metroid verifies as an exact match in `deep-gameplay-save-assisted-metroid-directdll2-20260530/deep-gameplay.csv`. Ruby's longer Flash128K route remains covered by the visual-save tooling but is not part of this strict deep suite yet because its current route budget is too slow for routine regression runs.
