...
Save-assisted deep gameplay coverage now has its own strict manifest, `docs/gba-save-assisted-deep-routes.csv`, using read-only approved save fixtures. The stable rows cover EEPROM (`zelda-minish-save-bedroom`, 16,000 frames, 26 snapshots, 10 distinct PCs; `mario-luigi-save-room`, 24,000 frames, 20 snapshots, 7 distinct PCs; `tony-hawk2-save-gameplay`, 15,000 frames, 25 snapshots, 9 distinct PCs), Flash64K (`advance-wars-save-map`, 15,000 frames, 25 snapshots, 11 distinct PCs; `mario-kart-save-race`, 7,200 frames, 12 snapshots, 8 distinct PCs), SRAM (`metroid-fusion-save-gameplay`, 30,000 frames, 25 snapshots, 17 distinct PCs), Flash128K (`pokemon-ruby-save-bedroom`, 6,000 frames, 10 snapshots, 7 distinct PCs), and a save-loaded Sonic timing/rendering anchor (`sonic-advance-save-demo`, 3,600 frames, 12 snapshots, 8 distinct PCs). The suite wrapper accepts alternate ROM/baseline roots, emits a contact sheet automatically, and now warns on low-diversity rows below a configurable distinct-PC threshold without failing strict baseline verification. The strict run `deep-gameplay-save-assisted-suite-deepened-20260601/deep-gameplay.csv` verifies 8/8 exact baseline matches, and `deep-gameplay-save-assisted-suite-deepened-20260601/contact-sheet.png` visually confirms the Zelda bedroom, Advance Wars map, Metroid boss-room, Sonic title-demo gameplay, Mario & Luigi room, deepened Tony Hawk 2 skate tutorial, Pokemon Ruby bedroom movement, and Mario Kart race checkpoints. The low-diversity report now flags only Ruby and Mario & Luigi for future route deepening. `scripts/new-deep-gameplay-frame-candidates.py` captures candidate stop frames for a manifest row and generates a contact sheet before promotion, kills its active `dotnet` child process tree on timeout/interruption, and caught title/publisher screens during Sonic route selection, a later dialogue bubble during Mario & Luigi route selection, and Ruby menu/title states before the shorter bedroom route was promoted.
