...
External reference comparison scaffolding now exists for checking gbaSharp frames against captures from independent emulators. `docs/gba-reference-frames.csv` tracks the starter targets, `scripts/compare-reference-frames.py` reports missing references, size mismatches, and pixel diffs, and `docs/reference-capture-workflow.md` documents the manual capture workflow. The initial anchors are Sonic Advance controlled gameplay, Pokemon Ruby bedroom movement, and Mario Kart race rendering; reference captures are intentionally local ignored artifacts under `reference-captures/`.
