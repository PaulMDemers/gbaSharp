...
- Expand `docs/gba-deep-gameplay-routes.csv` from generic route smokes into game-specific scripts that reach deterministic playable scenes, then promote stable final frames into visual baselines. Highest-value route fixes are `Mario Kart` to an active race, `F-Zero GP Legend` past name entry into a race, `Zelda Minish Cap` past file select, and `GTA` past save-slot/name setup.
