...
Save-assisted deep gameplay coverage now has its own strict manifest, `docs/gba-save-assisted-deep-routes.csv`, using read-only approved save fixtures. The stable rows cover EEPROM (`zelda-minish-save-bedroom`, 16,000 frames, 26 snapshots, 10 distinct PCs; `mario-luigi-save-room`, 24,000 frames, 20 snapshots, 7 distinct PCs; `tony-hawk2-save-gameplay`, 9,000 frames, 15 snapshots, 7 distinct PCs), Flash64K (`advance-wars-save-map`, 15,000 frames, 25 snapshots, 11 distinct PCs), SRAM (`metroid-fusion-save-gameplay`, 30,000 frames, 25 snapshots, 17 distinct PCs), Flash128K (`pokemon-ruby-save-bedroom`, 3,600 frames, 6 snapshots, 5 distinct PCs), and a save-loaded Sonic timing/rendering anchor (`sonic-advance-save-demo`, 3,600 frames, 12 snapshots, 8 distinct PCs). The suite wrapper accepts alternate ROM/baseline roots and emits a contact sheet automatically. The strict run `deep-gameplay-save-assisted-suite-ruby-20260531/deep-gameplay.csv` verifies 7/7 exact baseline matches, and `deep-gameplay-save-assisted-suite-ruby-20260531/contact-sheet.png` visually confirms the Zelda bedroom, Advance Wars map, Metroid boss-room, Sonic title-demo gameplay, Mario & Luigi room, Tony Hawk 2 skate tutorial, and Pokemon Ruby bedroom checkpoints. `scripts/new-deep-gameplay-frame-candidates.py` captures candidate stop frames for a manifest row and generates a contact sheet before promotion, kills its active `dotnet` child process tree on timeout/interruption, and caught title/publisher screens during Sonic route selection, a later dialogue bubble during Mario & Luigi route selection, and Ruby menu/title states before the shorter bedroom route was promoted.
