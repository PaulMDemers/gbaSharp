...
The first PSG pass adds cycle-advanced square channel 1/2 sampling for trigger, duty, frequency, length counters, envelope volume progression, master enable, and `SOUNDCNT_L` routing/volume. The desktop audio sink now subscribes to those PSG samples through a tested `PsgPcmResampler`, making basic square-channel tones audible alongside direct sound. Sweep, wave RAM, and noise remain open.
