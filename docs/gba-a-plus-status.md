...
The first PSG pass adds cycle-advanced square channel 1/2 sampling for trigger, duty, frequency, length counters, envelope volume progression, square-1 sweep overflow/update behavior, basic wave-channel wave RAM playback/output level, master enable, and `SOUNDCNT_L` routing/volume. The desktop audio sink now subscribes to those PSG samples through a tested `PsgPcmResampler`, making square/wave PSG tones audible alongside direct sound. Wave bank/dimension edge cases and noise remain open.
