...
Save-assisted deep gameplay coverage now has its own strict manifest, `docs/gba-save-assisted-deep-routes.csv`, using read-only approved save fixtures. The first stable rows cover EEPROM (`zelda-minish-save-bedroom`, 16,000 frames, 26 snapshots, 10 distinct PCs), Flash64K (`advance-wars-save-map`, 15,000 frames, 25 snapshots, 11 distinct PCs), and SRAM (`metroid-fusion-save-gameplay`, 30,000 frames, 25 snapshots, 17 distinct PCs). The suite wrapper now accepts alternate ROM/baseline roots and emits a contact sheet automatically. The strict run `deep-gameplay-save-assisted-suite-final-20260530/deep-gameplay.csv` verifies 3/3 exact baseline matches, and `deep-gameplay-save-assisted-suite-final-20260530/contact-sheet.png` visually confirms the Zelda bedroom, Advance Wars map, and Metroid boss-room checkpoints. Ruby's longer Flash128K route remains covered by the visual-save tooling but is not part of this strict deep suite yet because its current route budget is too slow for routine regression runs.
