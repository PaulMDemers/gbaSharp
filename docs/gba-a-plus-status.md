...
The current 44-route deep-gameplay rollup is `artifacts/deep-gameplay-current-44-rollup-20260628/summary.md`. It covers 44/44 manifest rows with 0 missing routes, 0 failing rows, and 0 low-diversity warnings after aligning route-specific thresholds to the current repeated evidence. The rollup now reports 44 strict `pass, match` rows.
