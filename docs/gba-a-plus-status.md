...
Longer non-baseline gameplay soak coverage now starts in `docs/gba-longplay-routes.csv`, with status tracked in `docs/gba-longplay-status.md` and a wrapper at `scripts/run-longplay-suite.ps1`. The fresh full-suite artifact `artifacts/longplay-suite-full7-20260604` covers Sonic Advance, Mario Kart, Metroid Fusion, Tony Hawk Pro Skater 2, Castlevania Aria, Castlevania Harmony, and Pokemon Ruby with 7/7 pass rows, 9-17 distinct PCs, 0 low-diversity warnings, and contact-sheet-confirmed gameplay scenes. The next-batch rollup `artifacts/longplay-nextbatch-rollup-20260604` adds Doom, GTA, Golden Sun, Mario & Luigi, Mega Man Battle Network, Wario Land 4, F-Zero GP Legend, and F-Zero Maximum Velocity with 8/8 pass rows and 0 route-threshold diversity warnings; the F-Zero rows were then retuned to shorter active-race final frames after candidate sweeps showed the longer windows ended in failure/loss states, GTA was retuned from a story-dialogue final frame to an active top-down gameplay frame, and Mario & Luigi was retuned from the find-Mario prompt to a clean room scene. Mega Man Battle Network is still a route-tightening candidate before baseline promotion. The longplay runner now supports resumable suite completion and route child-process timeout cleanup after interrupted runs.
...
External reference comparison scaffolding now exists for checking gbaSharp frames against captures from independent emulators. `docs/gba-reference-frames.csv` tracks all 8 save-assisted gameplay routes as reference targets, `scripts/new-reference-capture-checklist.py` joins those targets to the save-assisted route manifest for a concrete capture checklist, `scripts/validate-reference-captures.py` validates dropped PNGs before strict comparison, `scripts/compare-reference-frames.py` reports missing references, size mismatches, and pixel diffs, and the comparer can emit an actual/reference/diff contact sheet for visual review. `scripts/run-reference-dashboard.ps1` wraps the checklist, validation, comparison, contact sheet, and Markdown summary into a single command, with an optional save-assisted deep gameplay refresh. `docs/reference-capture-workflow.md` documents the manual capture workflow, and `docs/gba-reference-status.md` records the latest capture intake state. The 2026-06-03 post-audio dashboard check reports 8/8 missing mGBA reference PNGs, so strict pixel comparison is ready but blocked on local external captures. The anchors cover Zelda Minish Cap, Advance Wars, Metroid Fusion, Sonic Advance, Mario & Luigi, Tony Hawk Pro Skater 2, Pokemon Ruby, and Mario Kart; reference captures are intentionally local ignored artifacts under `reference-captures/`.
