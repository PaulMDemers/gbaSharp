...
Longer non-baseline gameplay soak coverage now starts in `docs/gba-longplay-routes.csv`, with status tracked in `docs/gba-longplay-status.md` and a wrapper at `scripts/run-longplay-suite.ps1`. The expanded post-audio longplay smoke covers Sonic Advance, Mario Kart, Metroid Fusion, Tony Hawk Pro Skater 2, Castlevania Aria, Castlevania Harmony, and Pokemon Ruby with 7/7 pass rows, 9-17 distinct PCs, and contact-sheet-confirmed gameplay scenes.
