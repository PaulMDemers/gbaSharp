...
Deep gameplay smoke tooling now exists for longer route-oriented playability checks. `scripts/run-deep-gameplay.ps1` drives a manifest of representative retail routes, writes final PPM frames, periodic state snapshots, and a CSV status report. The starter manifest is `docs/gba-deep-gameplay-routes.csv`. The first real-BIOS aligned starter run (`deep-gameplay-starter-20260524/deep-gameplay.csv`) passes 12/12 routes to frame 3,600: Doom, GTA, Sonic Advance, Mario Kart, Metroid Fusion, Zelda Minish Cap, Golden Sun, Advance Wars, Castlevania Aria of Sorrow, F-Zero GP Legend, Tony Hawk's American Sk8land, and Tony Hawk's Underground 2. This is still a smoke route, not proof of perfect gameplay, but it moves the signal beyond title-screen boot checks. The runner now also supports local baseline promotion and exact baseline verification through `-UpdateBaselines` and `-FailOnBaselineDiff`. Contact-sheet inspection classified the initial target scenes as 1 gameplay route, 4 intro/tutorial/dialogue routes, and 7 menu/setup routes. Route tightening has now upgraded all 12 starter routes into gameplay-scene baselines, and the full strict real-BIOS pass `deep-gameplay-strict-full-gameplay-20260526/deep-gameplay.csv` verifies 12/12 pass, 12/12 exact baseline matches, and 12/12 `targetScene=gameplay` rows: Doom, GTA, Sonic Advance, Mario Kart, Metroid Fusion, Zelda Minish Cap, Golden Sun, Advance Wars, Castlevania Aria of Sorrow, F-Zero GP Legend, Tony Hawk's American Sk8land, and Tony Hawk's Underground 2. The first expansion batch adds 4 more strict gameplay routes in `deep-gameplay-expansion-4-strict-20260526/deep-gameplay.csv`: Super Mario Advance 2, Banjo-Pilot, Mega Man Battle Network, and Advance Wars 2 all pass with exact local baseline matches. The second expansion batch adds Riviera, Summon Night, Crash Bandicoot - The Huge Adventure, and Powerpuff Girls - Mojo Jojo A-Go-Go in `deep-gameplay-expansion-8-strict-20260526/deep-gameplay.csv`, also 4/4 pass with exact baseline matches. The third expansion batch adds Scooby-Doo Unmasked and Spy Muppets in `deep-gameplay-expansion-scooby-muppets-strict-20260526/deep-gameplay.csv`, 2/2 pass with exact baseline matches. The fourth expansion route adds Castlevania Circle of the Moon; baseline promotion `deep-gameplay-castlevania-circle-strict-20260527/deep-gameplay.csv` passes 1/1, and strict verification `deep-gameplay-castlevania-circle-strict-20260527-verify/deep-gameplay.csv` reports 1/1 exact baseline match. The fifth expansion route adds Castlevania Harmony of Dissonance; baseline promotion `deep-gameplay-castlevania-harmony-strict-20260528/deep-gameplay.csv` passes 1/1, and strict verification `deep-gameplay-castlevania-harmony-strict-20260528-verify/deep-gameplay.csv` reports 1/1 exact baseline match. The sixth expansion batch adds Wario Land 4, Kirby Nightmare in Dream Land, and Kirby & The Amazing Mirror; baseline promotion `deep-gameplay-platformer-batch-strict-20260528/deep-gameplay.csv` passes 3/3, and strict verification `deep-gameplay-platformer-batch-strict-20260528-verify/deep-gameplay.csv` reports 3/3 exact baseline matches. The seventh expansion batch adds Crash Bandicoot 2 N-Tranced, F-Zero Maximum Velocity, and Crash Nitro Kart; baseline promotion `deep-gameplay-thirty-route-batch-strict-20260529/deep-gameplay.csv` passes 3/3, and strict verification `deep-gameplay-thirty-route-batch-strict-20260529-verify/deep-gameplay.csv` reports 3/3 exact baseline matches. A full 30-route real-BIOS audit on 2026-05-29 verifies exact matches for routes 1-24 in chunks (`deep-gameplay-full30-realbios-strict-20260529-01-10`, `-08`, `-09-retry`, `-11-15`, `-16-20`, and `-21-25`), then refreshes and verifies routes 25-30 under real BIOS in `deep-gameplay-realbios-refresh-20260529-25-30` and `deep-gameplay-realbios-refresh-20260529-25-30-verify`. The runner now auto-discovers the local real BIOS and sizes each route process timeout from the manifest `maxSeconds` value to avoid accidental no-BIOS or premature-timeout false failures; `-RouteMaxSecondsCap` can intentionally cap per-route wall time for bounded local smoke passes. `scripts/run-reference-dashboard.ps1` also accepts `-AllowDeepGameplayFailures` so deliberately capped refreshes can still write a complete dashboard summary. `scripts/run-deep-gameplay-suite.ps1` now orchestrates chunked full-suite runs, merges chunk CSVs, and writes status/low-diversity summaries; its full 30-route real-BIOS run `deep-gameplay-suite-full30-20260530/deep-gameplay.csv` verifies 30/30 pass rows with 30/30 exact baseline matches and 0 failing rows. `scripts/new-deep-gameplay-contact-sheet.py` turns any deep gameplay CSV into a labeled PNG contact sheet for fast visual review; `deep-gameplay-suite-full30-20260530/contact-sheet.png` visually confirms all 30 final frames. A 2026-05-30 snapshot-cadence quality pass keeps exact baselines while improving evidence on low-diversity routes: F-Zero GP 2->7 distinct PCs, Banjo-Pilot 3->5, Super Mario Advance 2 3->4, and Scooby 3->4; Kirby Nightmare remains at 3 because its active scene samples a tight main-loop PC range even with 50 snapshots. The main manifest now includes route-specific `minDistinctPcs` overrides for known stable low-diversity loops, including Sonic default at 7 PCs, Tony Hawk's American Sk8land at 6 PCs, Mega Man Battle Network at 5 PCs, Powerpuff Girls at 5 PCs, and Spy Muppets at 7 PCs. The post-audio strict refresh `artifacts/deep-gameplay-refresh-audio-pivot-01-04-thresholds` verifies Doom, GTA, Sonic Advance, and Mario Kart with 4/4 pass rows, 4/4 exact baseline matches, and 0 low-diversity warnings. The follow-up strict refresh `artifacts/deep-gameplay-refresh-audio-pivot-09-12` verifies Castlevania Aria, F-Zero GP, Tony Hawk's American Sk8land, and Tony Hawk's Underground 2 with 4/4 pass rows and 4/4 exact baseline matches; `artifacts/deep-gameplay-refresh-audio-pivot-sk8land-repeat` confirms Sk8land's 6-PC loop is repeatable. The 13-16 refresh `artifacts/deep-gameplay-refresh-audio-pivot-13-16` verifies Super Mario Advance 2, Banjo-Pilot, Mega Man Battle Network, and Advance Wars 2 with 4/4 pass rows and 4/4 exact baseline matches; `artifacts/deep-gameplay-refresh-audio-pivot-mmbn-repeat` confirms Battle Network's 5-PC room loop is repeatable. The 17-20 refresh `artifacts/deep-gameplay-refresh-audio-pivot-17-20` verifies Riviera, Summon Night, Crash Huge Adventure, and Powerpuff Girls with 4/4 pass rows and 4/4 exact baseline matches; `artifacts/deep-gameplay-refresh-audio-pivot-powerpuff-repeat` confirms Powerpuff's 5-PC action loop is repeatable. The 21-22 refresh `artifacts/deep-gameplay-refresh-audio-pivot-21-22` verifies Scooby-Doo Unmasked and Spy Muppets with 2/2 pass rows and 2/2 exact baseline matches; `artifacts/deep-gameplay-refresh-audio-pivot-spy-muppets-repeat` confirms Spy Muppets' 7-PC minigame loop is repeatable. The 23-24 Castlevania refresh verifies Circle of the Moon and Harmony of Dissonance with 2/2 pass rows and 2/2 exact baseline matches; Circle's manifest wall-clock budget is widened from 1,800 to 2,400 seconds after loaded-host runs reached frame 37,658/38,000 before the old cap, and `artifacts/deep-gameplay-refresh-audio-pivot-castlevania-circle-2400` confirms completion at frame 38,000 with 10 distinct PCs. The 25-28 platformer refresh verifies Wario Land 4, Kirby Nightmare in Dream Land, Kirby & The Amazing Mirror, and Crash 2 N-Tranced with 4/4 pass rows and 4/4 exact baseline matches; `artifacts/deep-gameplay-refresh-audio-pivot-crash-ntranced-repeat` confirms Crash N-Tranced's 7-PC side-scrolling loop is repeatable. The 29-30 racing refresh verifies F-Zero Maximum Velocity and Crash Nitro Kart with 2/2 pass rows and 2/2 exact baseline matches; `artifacts/deep-gameplay-refresh-audio-pivot-fzero-maximum-repeat` confirms F-Zero Maximum Velocity's 4-PC race loop is repeatable. Mario & Luigi still reaches the opening "find Mario" room/prompt and needs bespoke navigation scripting before promotion; Mega Man Battle Network 2 also remains in opening dialogue with the current probe. The suite is now 30 gameplay routes.

The post-audio Metroid Fusion solo refresh `artifacts/deep-gameplay-refresh-audio-pivot-metroid-solo/deep-gameplay.csv` reaches frame 42,000 under real BIOS with `pass`, exact baseline match, 17 snapshots, and 11 distinct PCs. This closes the long-route gap left by the smaller foreground chunks.

The current post-Tactics compatibility refresh reran the first deep-gameplay chunk under real BIOS. `artifacts/deep-gameplay-suite-current-20260625/deep-gameplay.csv` confirms Doom, GTA, Sonic Advance, and Mario Kart reach live gameplay scenes; Metroid Fusion reached frame 21,802 under a deliberately bounded 600-second foreground cap before being rerun as a solo long route. After visual review, GTA, Sonic Advance, Mario Kart, and Metroid Fusion baselines were refreshed to current active-gameplay frames. `artifacts/deep-gameplay-current-01-04-verify-20260625/deep-gameplay.csv` verifies Doom/GTA/Sonic/Mario Kart with 4/4 `pass, match` rows, and `artifacts/deep-gameplay-current-metroid-solo-verify-20260625/deep-gameplay.csv` verifies Metroid Fusion at frame 42,000 with 17 snapshots, 11 distinct PCs, and an exact baseline match.

The next current deep-gameplay band covers Zelda Minish Cap, Golden Sun, Advance Wars, Castlevania Aria of Sorrow, and F-Zero GP Legend. `artifacts/deep-gameplay-suite-current-06-10-20260625/chunk-06-10/deep-gameplay.csv` completed Zelda, Golden Sun, Advance Wars, and Aria before the suite-level timeout interrupted F-Zero; F-Zero then completed as a solo route. Golden Sun, Advance Wars, and F-Zero visual diffs were reviewed as active-scene drift and refreshed. `artifacts/deep-gameplay-current-07-08-verify-20260625/deep-gameplay.csv` verifies Golden Sun and Advance Wars with 2/2 `pass, match` rows, and `artifacts/deep-gameplay-current-fzero-gp-strict-20260625/deep-gameplay.csv` verifies F-Zero at frame 7,200 with an exact baseline match. F-Zero's current snapshot diversity is 5 distinct PCs versus the older 7-PC threshold, so it remains a repeatability/threshold follow-up rather than a visual failure.

The current 11-15 deep-gameplay band covers Tony Hawk's American Sk8land, Tony Hawk's Underground 2, Super Mario Advance 2, Banjo-Pilot, and Mega Man Battle Network. `artifacts/deep-gameplay-suite-current-11-15-20260625/chunk-11-15/deep-gameplay.csv` initially found a real-BIOS Banjo-Pilot crash at frame 237 with the old `030052F4` helper-prologue signature. The root cause was DMA enable rising-edge handling: the emulator reloaded stale IO-shadow source/destination addresses on re-enable, even though the internal DMA addresses had advanced and the game had not rewritten the address registers. Preserving internal source/destination until an address-register write restores the Banjo helper copy path. `artifacts/compat-banjo-realbios-after-dma-reenable-fix-20260625b.csv` verifies Banjo real-BIOS gameplay with 4/4 boot rows and 0 crashes, while `artifacts/compat-dma-reenable-regression-20260625.csv` keeps Legends of Wrestling II, Powerpuff Girls, Banjo-Pilot, Motocross Maniacs Advance, and Tactics Ogre Gaiden clean at 20/20 no-BIOS gameplay boot rows. After visual review, Super Mario Advance 2 and Banjo-Pilot baselines were refreshed as active-scene drift; `artifacts/deep-gameplay-current-sma2-banjo-strict-20260625/deep-gameplay.csv` verifies both with 2/2 `pass, match` rows. Banjo's current route diversity is 4 distinct PCs versus the older 5-PC threshold, so it remains a repeatability/threshold follow-up rather than a visual failure.

The current 16-20 deep-gameplay band covers Advance Wars 2, Riviera, Summon Night, Crash Bandicoot - The Huge Adventure, and Powerpuff Girls - Mojo Jojo A-Go-Go. `artifacts/deep-gameplay-suite-current-16-20-20260625/chunk-16-20/deep-gameplay.csv` reached target gameplay frames for all five routes; Summon Night and Crash matched existing baselines, while Advance Wars 2, Riviera, and Powerpuff needed route retuning rather than core fixes. `dump-frame --frame-dump FRAME:path.ppm` and input-script `repeat KEYS FIRST_FRAME INTERVAL_FRAMES COUNT DURATION` were added to make candidate-frame tuning cheaper. The retuned strict report `artifacts/deep-gameplay-current-16-20-retuned-strict-20260625/deep-gameplay.csv` verifies Advance Wars 2 at frame 30,000 with 14 distinct PCs, Riviera at frame 15,120 with 10 distinct PCs, and Powerpuff at frame 6,500 with 5 distinct PCs; all three are `pass, match`, and Powerpuff meets its 5-PC route threshold.

The current 21-25 deep-gameplay band covers Scooby-Doo Unmasked, Spy Muppets, Castlevania Circle of the Moon, Castlevania Harmony of Dissonance, and Wario Land 4. `artifacts/deep-gameplay-suite-current-21-25-20260626/chunk-21-25/deep-gameplay.csv` reaches all five target frames with 5/5 pass rows; Circle and Harmony matched existing baselines, while Scooby, Spy Muppets, and Wario Land 4 were same-scene visual drift and were refreshed after contact-sheet review. `artifacts/deep-gameplay-current-21-25-retuned-strict-20260626/deep-gameplay.csv` verifies Scooby, Spy Muppets, and Wario with 3/3 `pass, match` rows. Spy Muppets now repeats at 6 distinct PCs, so its route-specific threshold has been lowered from 7 to 6 PCs.

The current 26-30 deep-gameplay band covers Kirby Nightmare in Dream Land, Kirby & The Amazing Mirror, Crash 2 N-Tranced, F-Zero Maximum Velocity, and Crash Nitro Kart. `artifacts/deep-gameplay-suite-current-26-30-20260627/chunk-26-30/deep-gameplay.csv` reaches all five target frames with 5/5 pass rows; Kirby Nightmare and Crash N-Tranced matched existing baselines, while Kirby Amazing Mirror, F-Zero Maximum Velocity, and Crash Nitro Kart were same-scene visual drift and were refreshed after contact-sheet review. `artifacts/deep-gameplay-current-26-30-retuned-strict-20260627/deep-gameplay.csv` verifies the three refreshed rows with 3/3 `pass, match` rows: Kirby Amazing Mirror at 8 distinct PCs, F-Zero Maximum Velocity at its 4-PC route threshold, and Crash Nitro Kart at 10 distinct PCs.

The current 31-35 deep-gameplay band covers Metal Slug Advance, Wolfenstein 3D, Mega Man Zero, Street Fighter Alpha 3, and Puyo Pop. `artifacts/deep-gameplay-suite-current-31-35-20260628/deep-gameplay.csv` verifies all five routes with 5/5 `pass, match` rows and no low-diversity warnings. The contact sheet confirms in-game action/fight/puzzle scenes, with Metal Slug Advance and Mega Man Zero both meeting their 4-PC route thresholds while Wolfenstein 3D, Street Fighter Alpha 3, and Puyo Pop show broader PC diversity.

The current 36-40 deep-gameplay band covers Contra Advance, King of Fighters EX2, Fire Emblem Sacred Stones, Final Fantasy Tactics Advance, and GT Advance 2. `artifacts/deep-gameplay-suite-current-36-40-20260628/chunk-36-40/deep-gameplay.csv` completed the first four routes before the foreground shell timeout, all as `pass, match`; GT Advance 2 then completed solo in `artifacts/deep-gameplay-current-gt-advance2-solo-20260628/deep-gameplay.csv` with `pass, match`, 18,000 observed frames, and 9 distinct PCs. FFTA remains at its 4-PC route threshold, while the other completed rows show broader diversity.

The current 41-44 weak-row tail covers Alienators, Beyblade G-Revolution, Contra Hard Spirits JP, and Advance GTA JP. The first run `artifacts/deep-gameplay-suite-current-41-44-20260628/deep-gameplay.csv` showed Advance GTA JP was under-budgeted, timing out at frame 2,826 on the course-select screen before the next confirm tap. After widening only that row's wall-clock/step budget, `artifacts/deep-gameplay-suite-current-41-44-budgeted-20260628/deep-gameplay.csv` verifies all four rows as `pass, missing` exploratory checkpoints. Contact-sheet review promoted Contra Hard Spirits JP and Advance GTA JP into strict gameplay/progression baselines, and `artifacts/deep-gameplay-current-tail-promoted-strict-20260628/deep-gameplay.csv` verifies both as `pass, match`. Follow-up bespoke routes now also promote Alienators to a side-scrolling gameplay scene and Beyblade G-Revolution to its first room-control scene; `artifacts/deep-gameplay-current-alienators-gameplay-strict-20260628/deep-gameplay.csv` and `artifacts/deep-gameplay-current-beyblade-gameplay-strict-20260628/deep-gameplay.csv` verify both as `pass, match`.

The current 54-route deep-gameplay rollup is `artifacts/deep-gameplay-current-54-rollup-20260701/summary.md`. It covers 54/54 manifest rows with 0 missing routes, 0 failing rows, and 0 low-diversity warnings after aligning route-specific thresholds to the current repeated evidence. The rollup reports 54 strict `pass, match` rows. The expansion rows add Doom II at frame 6,000 as another first-person raycast/3D engine route, verified by `artifacts/deep-gameplay-current-doomii-gameplay-strict-20260628b/deep-gameplay.csv` with 8 distinct PCs; Chessmaster at frame 4,800 as an EEPROM board-game UI/rendering route, verified by `artifacts/deep-gameplay-current-chessmaster-gameplay-strict-20260628/deep-gameplay.csv` with 4 distinct PCs; Dragon Ball Z Collectible Card Game at frame 9,600 as an EEPROM strategy-card hand/UI route, verified by `artifacts/deep-gameplay-current-dbz-ccg-gameplay-strict-20260629/deep-gameplay.csv` with 6 distinct PCs; FIFA Soccer 2003 at frame 12,000 as an active sports-field route, verified by `artifacts/deep-gameplay-current-fifa2003-gameplay-strict-20260629/deep-gameplay.csv` with 7 distinct PCs; Madden NFL 2003 at frame 9,000 as an active football-field route, verified by `artifacts/deep-gameplay-current-madden2003-gameplay-strict-20260629/deep-gameplay.csv` with 7 distinct PCs; FIFA Soccer 2004 at frame 12,000 as a second active soccer-field route, verified by `artifacts/deep-gameplay-current-fifa2004-gameplay-strict-20260629/deep-gameplay.csv` with 8 distinct PCs; Driver 2 Advance at frame 9,000 as a broad-archive official 3D city/mission route, verified by `artifacts/deep-gameplay-current-driver2-gameplay-strict-20260629/deep-gameplay.csv` with 10 distinct PCs; Lara Croft Tomb Raider - The Prophecy at frame 6,000 as a broad-archive official action-adventure route, verified by `artifacts/deep-gameplay-current-tombraider-prophecy-gameplay-strict-20260629/deep-gameplay.csv` with 4 distinct PCs; Madden NFL 2004 at frame 9,000 as an on-field coin-toss/play-setup route, verified by `artifacts/deep-gameplay-current-madden2004-gameplay-strict-20260630/deep-gameplay.csv` with 7 distinct PCs; and Baldur's Gate - Dark Alliance at frame 21,000 as a clean in-engine dungeon/HUD route, verified by `artifacts/deep-gameplay-current-baldur-gameplay-strict-20260701/deep-gameplay.csv` with 9 distinct PCs. DemiKids Light and Tomb Raider Legend remain route-followups: DemiKids still lands in opening story scenes, and Tomb Raider Legend remains in opening story/dialogue.

The compatibility-finish refresh expands and reconsolidates that evidence at
`artifacts/compat-finish-standard-55-rollup-20260711`. The current manifest is
55/55 strict `pass, match`, with 0 missing routes, 0 failing rows, and 0
low-diversity warnings. Driv3r is the 55th active-gameplay route. The final
five-route band also reconfirms Driver 2 Advance, Tomb Raider: The Prophecy,
Madden NFL 2004, and Baldur's Gate. Dragon Ball Z CCG was independently aligned
...
The current external cross-emulator check is also green. The strict mGBA suite
at `artifacts/compat-finish-strict-reference-retimed-20260711` passes all 17
comparisons after restoring Mario Kart's documented gbaSharp 420/mGBA 435
alignment and retiming F-Zero Maximum from frame 7,200 to the window-matched
frame 7,197. F-Zero's closest current frame differs from mGBA by only 55 pixels
with SSIM 0.992718.
...
Save-assisted deep gameplay coverage now has its own strict manifest, `docs/gba-save-assisted-deep-routes.csv`, using read-only approved save fixtures. The stable rows cover EEPROM (`zelda-minish-save-bedroom`, 16,000 frames, 26 snapshots, 10 distinct PCs; `mario-luigi-save-room`, 24,000 frames, 20 snapshots, 7 distinct PCs; `tony-hawk2-save-gameplay`, 15,000 frames, 25 snapshots, 9 distinct PCs), Flash64K (`advance-wars-save-map`, 15,000 frames, 25 snapshots, 11 distinct PCs; `mario-kart-save-race`, 7,200 frames, 12 snapshots, 8 distinct PCs), SRAM (`metroid-fusion-save-gameplay`, 30,000 frames, 25 snapshots, 17 distinct PCs), Flash128K (`pokemon-ruby-save-bedroom`, 6,000 frames, 10 snapshots, 8 distinct PCs), and a controlled Sonic timing/rendering anchor (`sonic-advance-save-controlled`, 9,600 frames, 32 snapshots, 8 distinct PCs). The suite wrapper accepts alternate ROM/baseline roots, emits a contact sheet automatically, and warns on rows below either the global distinct-PC threshold or a route-specific `minDistinctPcs` value. The strict run `deep-gameplay-save-assisted-suite-sonic-controlled-20260601/deep-gameplay.csv` verifies 8/8 exact baseline matches with 0 low-diversity warnings. The matching contact sheet visually confirms the Zelda bedroom, Advance Wars map, Metroid boss-room, controlled Sonic beach gameplay, Mario & Luigi room, deepened Tony Hawk 2 skate tutorial, Pokemon Ruby bedroom movement, and Mario Kart race checkpoints. `scripts/new-deep-gameplay-frame-candidates.py` captures candidate stop frames for a manifest row and generates a contact sheet before promotion, kills its active `dotnet` child process tree on timeout/interruption, and caught title/publisher/demo screens during Sonic route selection before the controlled route was promoted.

External reference comparison scaffolding now exists for checking gbaSharp frames against captures from independent emulators. `docs/gba-reference-frames.csv` tracks all 8 save-assisted gameplay routes as reference targets, and `docs/gba-longplay-reference-frames.csv` tracks all 17 strict longplay targets. `scripts/new-reference-capture-checklist.py` joins those targets to the matching route manifest for a concrete capture checklist, `scripts/validate-reference-captures.py` validates dropped PNGs before strict comparison, `scripts/compare-reference-frames.py` reports missing references, size mismatches, and pixel diffs, and the comparer can emit an actual/reference/diff contact sheet for visual review. `scripts/run-reference-dashboard.ps1` wraps the default save-assisted checklist, validation, comparison, contact sheet, and Markdown summary into a single command, with an optional save-assisted deep gameplay refresh. `docs/reference-capture-workflow.md` documents the manual capture workflow, and `docs/gba-reference-status.md` records the latest capture intake state. The 2026-06-06 tranche-3 longplay reference check at `artifacts/longplay-reference-checklist-20260606-tranche3` reports 17/17 missing mGBA longplay PNGs, so strict pixel comparison for those rows is wired but blocked on local external captures. The original save-assisted anchors cover Zelda Minish Cap, Advance Wars, Metroid Fusion, Sonic Advance, Mario & Luigi, Tony Hawk Pro Skater 2, Pokemon Ruby, and Mario Kart; reference captures are intentionally local ignored artifacts under `reference-captures/`.
...
`scripts/audio-csv-to-wav.py` can also turn direct-sound and/or PSG CSVs into a stereo PCM WAV; use `--mix other.csv` to combine direct and PSG captures into one audible smoke artifact. It now uses the same default direct/PSG scales and 0.5 master headroom gain as the desktop mixed resampler, and advances on a shared event-driven GBA-cycle timeline. This remains diagnostic because the capture CSVs only contain emitted audio events, but it keeps smoke WAVs aligned with desktop audio assumptions.
...
`scripts/run-audio-smoke.ps1` now drives repeatable retail audio captures from `docs/gba-audio-smoke-routes.csv`, writing final frames, direct/PSG CSVs, per-stream summaries, mixed WAVs, logs, a CSV report, and a Markdown summary with WAV frame/duration plus peak/RMS/clipping metrics. The starter no-BIOS run `artifacts/audio-smoke-script-check-2` passes Sonic Advance and Pokemon Ruby to frame 300: Sonic reports 104,194 direct-sound samples at ~10.5 kHz and Ruby reports 133,502 direct-sound samples at ~13.4 kHz, with both early windows producing 0 PSG samples. PSG CSVs intentionally contain emitted mixed PSG rows only, not silent 32.768 kHz ticks.

The audio smoke manifest now includes save-assisted gameplay routes for Zelda Minish Cap, Advance Wars, Metroid Fusion, Sonic Advance, Mario Kart, Mario & Luigi, Tony Hawk 2, and Castlevania Aria/Harmony. The expanded manifest is clean through frame 1,800 for all 9 gameplay routes plus the 2 short title/intro routes. Zelda, Sonic controlled gameplay, Mario Kart, Mario & Luigi, and Castlevania Harmony exercise mixed direct-sound plus PSG output; Advance Wars, Metroid, Tony Hawk 2, and Castlevania Aria are direct-sound-only in these windows. PSG cadence summaries report the expected 512-cycle/32.768 kHz mode delta whenever PSG is active. The post-shared-mixer smoke batch `artifacts/audio-smoke-shared-mixer-batch` passes Sonic controlled gameplay, Mario Kart, and Mario & Luigi with mixed direct+PSG WAVs. After adding 0.5 master headroom, the Mario Kart balance check `artifacts/audio-smoke-headroom-report-mariokart` reports 75.000% peak, 14.279% RMS, and 0 clipped samples. The final compact balance pass covers direct-only Advance Wars/Metroid plus mixed Mario Kart/Castlevania Harmony; all four pass with 0 clipped samples and peaks between 37.500% and 75.000%.

The WinForms desktop frontend now has an opt-in toolbar audio toggle backed by a bounded WaveOut sink. It subscribes to the core direct-sound and PSG sample streams, feeds them into a shared cycle-timed `MixedPcmResampler` with 0.5 master headroom, outputs bounded 44.1 kHz stereo PCM, and clears queued audio on pause/reset.

The first PSG pass adds cycle-advanced square channel 1/2 sampling for trigger, duty, frequency, length counters, envelope volume progression, square-1 sweep overflow/update behavior, basic wave-channel wave RAM playback/output level, basic noise-channel LFSR output, master enable, and `SOUNDCNT_L` routing/volume. `SOUNDCNT_X` readback now preserves the writable master-enable bit while reporting live PSG channel active flags, and disabling master sound clears tracked PSG activity. Wave-channel emulation now models hidden two-bank wave RAM, CPU access to the non-playback bank, selected-bank playback, 64-sample dimension mode, length expiration, and the forced 75% volume bit. Noise-channel clocking now uses the 4.194304 MHz legacy sound clock divided by the NR43 divisor/shift period, with tests pinning 15-bit and 7-bit LFSR sample sequences. The desktop audio sink now mixes PSG and direct-sound events on one GBA-cycle timeline, making square/wave/noise PSG tones audible alongside direct sound without the two streams appending independent chunks.
...
Earlier named failures remain useful anchors for root-cause work, but several have been cleared under current real-BIOS gameplay: Banjo Pilot, GTA, Riviera, Powerpuff Girls - Mojo Jojo A-Go-Go, Muppets - On with the Show, Scooby-Doo! - Unmasked, Spy Muppets, both Crash/Spyro superpacks, and UMUSCLE/Kinnikuman. Remaining work should avoid treating those old signatures as current failures until they are reproduced again on the latest core. Track any post-DMA-fix invalid/null-PC rows from a refreshed archive slice and low-motion/manual-route candidates as the current core-bug queue. A few titles such as `Ace Combat Advance`, `MLB 2K7`, `ATV Thunder Ridge`, `Baldur's Gate - Dark Alliance`, `The Bible Game`, `Big Mutha Truckers`, `Boxing Fever`, `Bomberman Tournament`, `Broken Sword`, `Capcom Mini Mix`, `Cars`, `Classic NES Series` entries, `Catwoman`, `CIMA: The Enemy`, `CT Special Forces`, `Dark Arena`, `Defender of the Crown`, `DemiKids`, `Denkiblocks`, `Dexter's Laboratory Chess Challenge`, `Driver 3`, remaining `FIFA` entries, `Fear Factor - Unleashed`, `The Fairly OddParents! Clash with the Anti-World`, `Game & Watch Gallery 4`, `Gem Smashers`, `Inspector Gadget`, `Lego Racers 2`, remaining `Madden NFL` entries, `Tomb Raider - Legend`, and `Mary-Kate and Ashley - Girls Night Out` classify as `low-motion`/`stalled-late` under generic input, so they are candidates for manual/game-specific input scripts rather than immediate core fixes.
...
- Bitmap modes 3/4/5 now use the normal layered scanline compositor instead of directly painting raw bitmap VRAM. The bitmap BG respects BG2 enable/priority/window/blend rules, OBJ renders in bitmap modes, and bitmap-mode OBJ tiles below 512 are treated as transparent because hardware OBJ tile data starts at `06014000`. This fixes the vertical striping seen in `Baldur's Gate - Dark Alliance`; focused strict replay of Doom, Sonic Advance, Driver 2 Advance, and Tomb Raider Prophecy remains baseline-matched.
