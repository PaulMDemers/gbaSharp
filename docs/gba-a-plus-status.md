...
Longer non-baseline gameplay soak coverage now starts in `docs/gba-longplay-routes.csv`, with status tracked in `docs/gba-longplay-status.md` and a wrapper at `scripts/run-longplay-suite.ps1`. The current full-suite artifact `artifacts/longplay-suite-full15-20260605` verifies all 15 longplay routes with 15/15 pass rows, 0 failures, 0 low-diversity warnings, and contact-sheet-confirmed scenes. It covers Sonic Advance, Mario Kart, Metroid Fusion, Tony Hawk Pro Skater 2, Castlevania Aria, Castlevania Harmony, Pokemon Ruby, Doom, GTA, Golden Sun, Mario & Luigi, Mega Man Battle Network, Wario Land 4, F-Zero GP Legend, and F-Zero Maximum Velocity. The F-Zero rows were retuned to shorter active-race final frames after candidate sweeps showed the longer windows ended in failure/loss states, GTA was retuned from a story-dialogue final frame to an active top-down gameplay frame, Mario & Luigi was retuned from the find-Mario prompt to a clean room scene, and Mega Man Battle Network was retuned from PET dialogue to a clean room scene. Fire Emblem has now been added as the first post-15-suite expansion route and verifies a tactical map/grid scene at frame 36,000 with 9 distinct PCs. The longplay runner supports resumable suite completion and route child-process timeout cleanup after interrupted runs.
