...
The current 45-route deep-gameplay rollup is `artifacts/deep-gameplay-current-45-rollup-20260628/summary.md`. It covers 45/45 manifest rows with 0 missing routes, 0 failing rows, and 0 low-diversity warnings after aligning route-specific thresholds to the current repeated evidence. The rollup reports 45 strict `pass, match` rows. The expansion row adds Doom II at frame 6,000 as another first-person raycast/3D engine route, verified by `artifacts/deep-gameplay-current-doomii-gameplay-strict-20260628b/deep-gameplay.csv` with 8 distinct PCs.
